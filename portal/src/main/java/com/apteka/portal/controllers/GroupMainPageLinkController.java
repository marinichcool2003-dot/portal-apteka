...
public class GroupMainPageLinkController {
    ...
    @PostMapping
    public ResponseEntity<GroupMainPageLinksResponseDTO> create(@RequestBody GroupMainPageLinksRequestDTO dto, @AuthenticationPrincipal AppUserDetails currentUser) {
        return ResponseEntity.ok(groupMainPageLinksService.create(dto, currentUser));
    }

    @Operation(summary = "Изменить группу ссылок")
    @PreAuthorize("hasAnyRole('ADMIN', 'LINK_CHANGER')")
    @PutMapping("/{id}")
    public ResponseEntity<GroupMainPageLinksResponseDTO> update(@PathVariable Integer id, @RequestBody GroupMainPageLinksRequestDTO dto, @AuthenticationPrincipal AppUserDetails currentUser) {
