...
public class TaskController {
    ...
    @Operation(summary = "Задачи моей группы", description = "Возвращает задачи очереди группы-исполнителя (GroupTask.intendedGroup = группа текущего пользователя), с фильтрами.")
    ...
    public ResponseEntity<Page<TaskShortResponseDTO>> getTasksAssignedMyGroup(
        ...
        DepartamentTaskWithFiltersDTO filter = DepartamentTaskWithFiltersDTO.builder()
                .assignerGroupId(currentUser.getUserGroup().getId())
                .status(dto.status())
                .priority(dto.priority())
                .workTypeId(dto.workTypeId())
                .groupTaskId(dto.groupTaskId())
    ...
    @Operation(summary = "Расширенный фильтр задач", description = "Полная фильтрация: groupId = группа-исполнитель (intended), creatorGroupId = группа-создатель категории; также status, priority, creatorId, assignerId и др.")
    @PreAuthorize("@security.hasAction('CAN_SELECT_ANOTHER_GROUP_TASKS') or @security.hasRole('ADMIN')")
    ...
    public ResponseEntity<Page<TaskShortResponseDTO>> getDepartamentTaskWithFilters(
        ...
        DepartamentTaskWithFiltersDTO filter = DepartamentTaskWithFiltersDTO.builder()
                ...
                .workTypeId(dto.workTypeId())
                .groupTaskId(dto.groupTaskId())
                .assignerGroupId(dto.assignerGroupId())
                .creatorGroupId(dto.creatorGroupId())
