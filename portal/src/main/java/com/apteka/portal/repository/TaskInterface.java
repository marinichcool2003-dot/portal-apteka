...
public interface TaskInterface extends JpaRepository<Task, Long> {
    ...
    @Query("""
            ...
            ORDER BY t.creationDate DESC
            """)
    List<Task> findAptekiGroupTasksWithFilters(
            @Param("groupId") Integer groupId,
    ...
    @Query("""
            ...
            """)
    List<GroupTaskStatsDTO> getGroupClientStats();

    // Статистика по Группам Аптек
    @Query("""
                SELECT new com.apteka.portal.dtos.response.GroupTaskStatsDTO(
                    ga.id,
                    ga.name,
                ...
                FROM Task t
                JOIN t.assignedGroupApteki ga
                GROUP BY ga.id, ga.name
            """)
    List<GroupTaskStatsDTO> getGroupAptekiStats();
}
